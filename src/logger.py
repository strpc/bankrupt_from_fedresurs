...
logger.setLevel(eval('logging.' + DEBUG_MODE))  # or ERROR
...
def do_write_info(*args):
    logger.info(_do_msg(*args))


def do_write_debug(*args):
    logger.debug(_do_msg(*args))


def do_write_error(*args):
    logger.error(_do_msg(*args))


def do_write_warning(*args):
    logger.warning(_do_msg(*args))


def do_write_critical(*args):
    logger.critical(_do_msg(*args))
